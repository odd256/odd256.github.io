...
2. 通过 $z$，生成对应的 mean 和 variance，即找到对应的高斯分布
   ...
   > A：通过一个神经网络结构，学习到对应的 mean 和 variance：$\sigma \times z+\mu$
   > Q：为什么 $z$ 是一个高斯分布？
...
> [!warning]
> $L_{b}$ 表示其 $\log$ 似然的下边界，如果仅优化参数 $P(x|z)$ 对下边界求极大值，可能会导致边界变大，但实际似然估计值变小的情况
...
> [!note]
> 使 KL 散度最小就相当于让 $P(z)$ 和 $q(z|x)$ 的分布越相似
